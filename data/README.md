...
`legz_confidence` is the LEGZ baseline probability input. `jinx_input` is a signed JINX contextual adjustment in percentage points. `ljpc` is the canonical final L&J estimated hit probability. `lj_probability` and `lj_confidence` are compatibility aliases of `ljpc` and may not diverge from it. `legz_value` is the distinct 0–100 evidence-strength/predictability score. `economic_value` is a separate market/payout attractiveness score and never alters LJPC. `pom_value` combines prediction quality and economics using `0.80 × sqrt(legz_value × ljpc) + 0.20 × economic_value`. `lj_conviction` is a separate diagnostic/additive metric and is not substituted for probability.
