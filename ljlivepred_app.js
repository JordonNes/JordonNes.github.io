...
(() => {
  ...
  const isWatch = s => /WATCH|NO LIVE|DATA-LIMITED|^PASS\\b|PRICE WATCH/i.test(String(s || ""));
  const unique20 = rows => {
    const seen = new Set();
  ...
  function qcs(title,rows){
    return `<section class="section"><div class="section-head"><h2>${esc(title || "LIVE QUICKIES")}</h2><span class="muted">Same approved horizontal QC presentation • live data only</span></div>${rows && rows.length ? `<div class="qc-list">${rows.map(qcRow).join("")}</div>` : `<div class="status-panel"><b>NO VERIFIED LIVE EVENT</b><p>This sport page remains ready and will populate when an event is confirmed in progress.</p></div>`}${rules()}<div class="layout-seal">L&amp;J LIVE PRESENTATION LOCK • refresh data, not architecture</div></section>`;
  }
  function statusGrid(){
