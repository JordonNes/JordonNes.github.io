...
SRC=DATA/"performance_history.csv"; OUT=DATA/"history_coverage.json"

def main():
    by=defaultdict(lambda:{"facts":0,"players":set(),"events":set(),"dates":[],"metrics":set()})
    if SRC.exists() and SRC.stat().st_size:
        with SRC.open(newline="",encoding="utf-8-sig") as fh:
            for r in csv.DictReader(fh):
                lg=r.get("league") or "UNKNOWN"; x=by[lg]; x["facts"]+=1
    ...
    for lg,x in sorted(by.items()):
        leagues[lg]={
          ...
          "metrics":sorted(x["metrics"]),"earliest_event_date":min(x["dates"]) if x["dates"] else None,
