...
MAX_PLAYERS=max(1,int(os.getenv("ESPN_PLAYER_ENRICHMENT_MAX_PLAYERS","80")))
...
def main():
    ...
    # Stable ordering makes bounded runs deterministic.
    candidates.sort(key=lambda x:(x[0],norm(x[1])))
    for league,market_name,identity in candidates[:MAX_PLAYERS]:
        espn_id=str(identity["espn_id"]);resolved+=1
        key=(league,espn_id);prior=stored.get(key) or {}
        ...
        if successes:
            row["last_success_utc"]=NOW.isoformat();row["successful_resources"]=successes;refreshed+=1
            stored[key]=row
    output={
    ...
    temp.replace(OUT)
    print(f"ESPN player enrichment: active={len(active)} resolved={resolved} refreshed={refreshed} unresolved={len(unresolved)} errors={len(errors)}")
