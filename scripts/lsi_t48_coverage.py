...
def main():
    ...
    status="SLA_BREACH" if acquisition_gap or evaluated!=total else ("DEGRADED_MARKET_STALE" if stale_total else "PASS")
    if acquisition_gap:
    ...
    for league,agg in leagues.items():
        ...
        agg["status"]="SLA_BREACH" if (known and a==0) or e!=a else ("DEGRADED_MARKET_STALE" if agg.get("stale_props") else "PASS")
