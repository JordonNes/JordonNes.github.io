...
def build():
    ...
    qc_board = {"schema_version": "LJ-QC-PROP-BOARD-1", "events": []}
    if QC_BOARD.exists():
        try:
            qc_board = json.loads(QC_BOARD.read_text(encoding="utf-8"))
        except json.JSONDecodeError:
            pass
    js = (
        "/* Generated by scripts/lsi_pipeline.py; do not edit manually. */\n"
        "window.LSI_PR=" + json.dumps(payload, ensure_ascii=False, separators=(",", ":")) + ";\n"
        "window.LJ_QC_PROP_BOARD=" + json.dumps(qc_board, ensure_ascii=False, separators=(",", ":")) + ";\n"
    )
