(() => {
  ...
  async function load() {
    const sources = {
      ...
      feature: RAW + '/evaluation/feature_calibration.json',
  ...
  function styles() {
    ...
    const s=document.createElement('style'); s.id='lsi-status-style'; s.textContent=`
      ...
      .lsi-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:1px;background:rgba(255,255,255,.07)}
      ...
      .lsi-detail-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin:22px 0}.lsi-detail-card{border:1px solid #ddd;border-radius:14px;padding:16px;background:#fff}
      ...
      @media(max-width:1050px){.lsi-grid{grid-template-columns:repeat(2,1fr)}.lsi-detail-grid{grid-template-columns:1fr}.lsi-front-head{align-items:flex-start;flex-direction:column}}
    `; document.head.appendChild(s);
  }

  function compact(d) {
    const a=d.archive||{}, m=d.manifest||{}, e=d.evaluation||{}, f=d.feature||{}, s=d.settlement||{}, g=d.gate||{}, o=d.overlay||{};
    ...
    return `
      <section class="lsi-front" aria-label="LSI intelligence status">
        ...
        <div class="lsi-grid">
          ...
          <div class="lsi-cell"><h3>Feature Calibration</h3><div class="lsi-big">${fmt(f?.gate?.decisive_settled_with_feature_state||0)} / ${fmt(f?.gate?.minimum_decisive_settled_with_feature_state||500)}</div><div class="lsi-sub">Spectrum v3 settled feature states • ${esc(f.status||'shadow pending')}</div></div>
          <div class="lsi-cell"><h3>Live Influence</h3><div class="lsi-big">${enabled?'ON':'OFF'}</div><div class="lsi-sub">${enabled?'Only mature gated markets; ±3 max':'SHADOW ONLY — no learned feature weight changes affect current LJPC'}</div></div>
        </div>
  ...
  function full(d) {
    ...
    const a=d.archive||{},m=d.manifest||{},e=d.evaluation||{},eh=d.evalHealth||{},f=d.feature||{},s=d.settlement||{},g=d.gate||{},o=d.overlay||{},p=d.performance||{};
    ...
    root.innerHTML=`
      <main class="lsi-detail">
        ...
        <div class="lsi-detail-grid">
          ...
          <section class="lsi-detail-card"><h2>3. Feature Calibration</h2>${kv('Mode','SHADOW ONLY')}${kv('Settled Spectrum rows',fmt(f?.gate?.decisive_settled_with_feature_state||0)+' / '+fmt(f?.gate?.minimum_decisive_settled_with_feature_state||500))}${kv('Per-core-feature gate',fmt(f?.gate?.minimum_per_feature_samples||400))}${kv('Weight review',f?.gate?.eligible_for_weight_review?'ELIGIBLE':'LOCKED')}${kv('Weight changes authorized',f?.gate?.weight_change_authorized?'YES':'NO')}</section>
          <section class="lsi-detail-card"><h2>4. Controlled Learning</h2>${kv('Eligible markets',fmt(g.eligible_market_count||0))}${kv('Overlay enabled',o.enabled?'YES':'NO')}${kv('Max adjustment','±'+Number(o.max_abs_confidence_delta||3).toFixed(0)+' pts')}${kv('Live influence',o.enabled?'ACTIVE':'LOCKED')}${kv('Gate policy','200+ settled / 98% settlement / CLV / multi-source')}</section>
        </div>
